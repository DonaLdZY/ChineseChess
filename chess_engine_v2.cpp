...
const int INF=32767;
const int m1=10000007,m2=1000009;
...
const int f3[8][2]={{1,-2},{-1,-2},{-2,1},{-2,-1},{2,1},{2,-1},{-1,2},{1,2}};//跳马，马脚=f1[x/2]
...
class board{
  ...
  public:
    ...
    //位置x,y的必要条件:为空或对方棋子
    bool can_move(int x,int y){return g[x][y]==0||(g[x][y]&1==1);}
...
int board::solve(int (&bm)[4],int c=MaxDepth,int worst=3*INF){ 
    ...
    //记忆化搜索
    pair<int,int> hs=hash();
    if (track.count(hs)){
        if (track[hs].first>=c)
            return track[hs].second;
    }
